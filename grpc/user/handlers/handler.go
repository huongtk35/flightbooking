...
import (
	...
	"flightbooking/grpc/user/request"
	"flightbooking/util"
	"log"
	"net/http"
	"time"
...
func NewHandler(service repositories.IUserService) Handler {
...
func (h *Handler) HandleRegister(ctx *gin.Context) {
	...
	create, err := h.userService.Create(&db.User{
...
func (h *Handler) ValidatePassword(ctx *gin.Context) {
	req := request.ValidatePasswordRequest{}
	err := ctx.ShouldBind(&req)
	if err != nil {
		log.Println("Error binding request", err)
		util.SendDefaultBadRequestError(ctx)
