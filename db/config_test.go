...
func TestAutoBindConfig(t *testing.T) {
	...
	assert.Equal(t, "root", viper.GetString("postgres.username"))
	assert.Equal(t, "12345", viper.GetString("postgres.password"))
	assert.Equal(t, "bookingfly", viper.GetString("postgres.database"))
